...
const S  = 1085;                         // 盤（正方形）の1辺（盤空間px）
const RHO= 1.72;                         // 角マス : 通常マス（本家の実測比）
...
/* ══════════ マップ ══════════ */
const GCOL = ['#2A86C4','#4E9E22','#C9922C','#C2621B','#C43F6B','#7B4FBE','#C6321F'];
...
function drawBackdrop(ctx, map, T){
  ...
  // ① 上からの光。暗すぎる背景を一段持ち上げる
  const lg = ctx.createLinearGradient(0, 0, 0, SH);
  lg.addColorStop(0,   'rgba(196,226,250,0.40)');
  lg.addColorStop(0.45,'rgba(150,190,225,0.26)');
  lg.addColorStop(1,   'rgba(70,110,155,0.10)');
  ctx.save(); ctx.fillStyle = lg; ctx.fillRect(0,0,SW,SH);
  // ② 盤が載る地面。これが無いと盤だけが宙に浮いて見える
  const rx = SW*0.72, ry = rx*KSQ*0.92;
  const gg = ctx.createRadialGradient(BCX, BCY+30, rx*0.18, BCX, BCY+30, rx);
  gg.addColorStop(0,   map.deco==='ice' ? '#D6E7F2' : '#E3DCC6');
  gg.addColorStop(0.62,map.deco==='ice' ? '#9EC2DA' : '#B9AE8C');
  gg.addColorStop(1,   'rgba(90,130,170,0)');
  ctx.beginPath(); ctx.ellipse(BCX, BCY+30, rx, ry, 0, 0, Math.PI*2);
  ctx.fillStyle = gg; ctx.fill();
  ctx.restore();
}

function drawLake(ctx, map, T){
  const inset = TD + 110;   // マスの内側に縁を残す（本家は水面が盤幅の42%）
...
function drawTile(ctx, G, i, T){
  ...
  if(t.type==='city'){
    // 天面は必ず区画色。持ち主は外周の帯（PCOL）で示す＝本家の見え方
    const g = GCOL[t.g];
    top  = t.owner>=0 ? g : mixHex(g, '#FFFFFF', 0.36);
    side = toHex(shade(g, -0.42));
    band = t.owner>=0 ? PCOL[t.owner] : toHex(shade(g, -0.18));
  ...
  dvTile(ctx, shrinkQuad(q, 3), { top, side, h: t.corner ? TILE_H+4 : TILE_H,
                   band, bandQuad: band ? bandQuad(i, 0.20) : null,
