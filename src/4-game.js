...
let cfg = { mapId:'lgr', turns:30, timeLimit:1500, cash:3000000, ai:1, speed:1, n:4, cls:0, team:0,
...
function newGame(){
  ...
  // チーム戦：順番決めのあとも 🔴🔵🔴🔵 と交互に手番が回るように並べ替える（本家と同じ）
  let seatsIn = cfg.seats.slice(0,cfg.n);
  if(cfg.team){
    const A = seatsIn.filter(s=>s.team===0), B = seatsIn.filter(s=>s.team===1);
    if(A.length===2 && B.length===2) seatsIn = (seatsIn[0].team===0) ? [A[0],B[0],A[1],B[1]] : [B[0],A[0],B[1],A[1]];
  }
  G = {
    cls: cfg.cls, team: !!cfg.team, map, tiles: buildTiles(map),
    players: seatsIn.map((s,i)=>{
      ...
      return {
        name:(cfg.team ? (s.team ? '🔵' : '🔴') : '') + s.name, team: cfg.team ? (s.team|0) : -1,
        kind:s.kind, ch: card.art, card: card.id,
...
async function resolveInner(pi){
  ...
  if(t.type === 'start'){
    ...
    } else if(G.team && allied(G, t.owner, pi)){
      toast('L','🤝','味方の街', t.name+' は通行料なし', 1600); await wait(500);
    } else {
      await payToll(pi, i);
...
async function pickEnemyCity(pi, msg, pred, score){
  const ok = i => G.tiles[i].type==='city' && G.tiles[i].owner>=0 && !allied(G, G.tiles[i].owner, pi)
...
function checkWin(){
  ...
  if(G.team){
    const teams = new Set(alive.map(p=>G.players.indexOf(p)%2));
    if(teams.size === 1) return finish(G.players.indexOf(alive[0]),'相手チーム全員破産');
  } else if(alive.length === 1) return finish(G.players.indexOf(alive[0]),'独り勝ち');
...
function finish(pi, reason, col){
  ...
  showCelebrate(['おめでとうございます！', reason, winnerName(pi) + ' の勝ち！'], 2800);
  addFx('confetti', SW/2, 0, 3000, null, null, false, {scr:true, w:SW, h:SH, n:110});
  bgm('win');
  news('🏆 '+winnerName(pi)+' が「'+reason+'」で勝利！');
...
async function celebrate(pi, reason, col){
  ...
  $('#cel3').textContent = winnerName(pi) + ' の勝ち！';
...
function showResult(){
  ...
  bgm('result');
  const rk = rank();
  $('#resWin').textContent = winnerName(G.winner) + ' の勝利！';
  ...
  const meWon = meSeat>=0 && (G.winner===meSeat || (G.team && allied(G, G.winner, meSeat)));
  grantRewards(meWon, (meWon && G.winX) || 1, meSeat>=0 ? prizeFor(meSeat) : 0);
}
...
function timeUp(){
  const rk = rank();
  if(G.team){   // チーム戦は合計資産で決める。勝ちチームの上位が代表
    const ta = [0,0]; rk.forEach(r=>{ if(r.a>0) ta[r.i%2] += r.a; });
    const best = ta[0] >= ta[1] ? 0 : 1;
    const top = rk.find(r=>r.i%2===best) || rk[0];
    return finish(top.i, 'ターン終了・チーム総資産1位');
  }
  finish(rk[0].i, 'ターン終了・総資産1位');
}
/* 勝者の呼び名（チーム戦は「チーム」で呼ぶ） */
function winnerName(pi){
  const p = G.players[pi];
  if(!G.team) return p.name;
  const mates = G.players.filter((q,i)=>i%2===pi%2).map(q=>q.name.replace(/^(🔴|🔵)/,''));
  return (p.team ? '🔵チーム' : '🔴チーム')+'（'+mates.join('・')+'）';
}
...
function takeRoll(pi){
  ...
  return new Promise(res=>{
    ...
    const clear = ()=>{ $('#push').onclick=null; $('#odd').onclick=null; $('#even').onclick=null;
      $('#skillBtn').onclick=null; clearTimeout(autoTm); };
    // 20秒放置で自動ロール（本家の制限時間）
    autoTm = setTimeout(()=>{ if($('#push').onclick){ toast('R','⏱','時間切れ','自動でサイコロを振ります',1500); done(null); } }, 20000*SPEED);
...
async function aiBuy(pi, i){
  ...
  const block = G.players.some((q,qi)=> !allied(G,qi,pi) && !q.out &&
    CITY_SLOTS[t.g].filter(j=>allied(G,G.tiles[j].owner,qi)).length >= 2);
...
$('#optTeam').onchange    = e => { cfg.team = +e.target.value; if(cfg.team){ cfg.n = 4; const n=$('#optN'); if(n) n.value='4'; } renderSeats(); };
$('#optSpeed').onchange   = e => { cfg.speed = +e.target.value; SPEED = cfg.speed; };

async function launch(){
  SPEED = cfg.speed;
  if(cfg.team){ cfg.n = 4; cfg.seats.forEach((s,i)=>{ s.team = i%2; }); }   // チーム戦は必ず 2:2。組は設定画面の並びで固定
  const C = classOf(cfg.cls);
  if(C.fee > SV.gold){
...
setOpt('#optTeam',    cfg.team,      v => cfg.team = +v);
